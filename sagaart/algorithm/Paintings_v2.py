...
category, year, height, width, work_material, pad_material, count_title, count_artist, country, sex, solo_shows, group_shows, age, is_alive = "Печать", 1983, 60.5, 91.0, "Цветная литография", "Сотканная бумага", np.NaN, np.NaN, "Россия", "М", "centre pompidou, whitney museum of american art, the metropolitan museum of art, los angeles county museum of art (lacma)", "whitney museum of american art, the metropolitan museum of art, los angeles county museum of art (lacma)", 80, np.NaN
...
data = [category, year, height, width, work_material, pad_material, count_title, count_artist, country, sex, solo_shows, group_shows, age, is_alive]
...
def make_shows_authority_from_shows(shows: str) -> float:
    ...
    """
    all_shows = ['centre pompidou', 'whitney museum of american art', 'the metropolitan museum of art', 'los angeles county museum of art (lacma)', 'guggenheim museum bilbao', 'louisiana museum of art', 'hirshhorn museum and sculpture garden', 'museum of contemporary art', 'los angeles (moca)', 'tate britain', 'museum ludwig', 'national gallery of victoria', 'hamburger bahnhof', 'neue nationalgalerie', 'national portrait gallery - london', 'art institute of chicago', 'national museum of modern and contemporary art - korea (mmca)', 'museo tamayo', 'tel aviv museum of art', 'tate liverpool', 'international center of photography (icp)', 'mca chicago', 'new museum', 'dallas museum of art', 'brooklyn museum', 'museum of modern art (moma)', 'tate modern', 'solomon r. guggenheim museum', 'national gallery of art', 'washington', 'd.c.', 'ullens center for contemporary art (ucca)', 'san francisco museum of modern art (sfmoma)', 'perez art museum miami (pamm)', 'mass moca', 'museo reina sofia', 'moma ps1', 'serpentine galleries', "museu d'art contemporani de barcelona (macba)", 'jewish museum', 'k20 grabbeplatz', 'dia:beacon', 'museum fur moderne kunst', 'frankfurt (mmk)', 'museum of contemporary art australia (mca)', 'institute of contemporary art', 'miami (ica miami)', 'aspen art museum', 'schirn kunsthalle frankfurt', 'dallas contemporary', 'hammer museum', 'garage museum of contemporary art', 'deichtorhallen hamburg', 'yuz museum shanghai', 'mori art museum', 'the broad', 'tai kwun', 'fondation beyeler', 'malba', 'boston', 'stedelijk museum amsterdam', 'castello di rivoli', 'leeum - samsung museum of art', 'dia:chelsea', 'kunstmuseum basel', 'power station of art', 'museo jumex', 'met breuer', 'lenbachhaus', 'palazzo grassi - punta della dogana', 'nasher sculpture center', 'haus der kunst', 'institute of contemporary arts', 'london', 'whitechapel gallery', 'secession', 'kunsthalle basel', 'm+', "museo d'arte contemporanea di roma (macro)", 'kroller-muller museum', 'fondazione prada', 'martin-gropius-bau', 'the bass museum of art', 'palais de tokyo', 'rockbund art museum', 'studio museum in harlem', 'national gallery singapore', 'k21 standehaus', 'kw institute for contemporary art', 'jeu de paume', 'zeitz mocaa', 'museum of old and new art', 'musée du louvre', 'museu de arte moderna de sao paulo (mam)', 'museu de arte moderna (mam rio)']
    n = len(all_shows)
...
def preprocess(data: list) -> np.ndarray:
    ...
    """
    return np.array(data[:-4] + [make_shows_authority_from_shows(data[-4]), make_shows_authority_from_shows(data[-3])] + data[-2:])


# model = CatBoostRegressor.load_model("/Users/dadaya/Desktop/catboost_v1.json", format = "json")
# выберите путь до файла
# print(model.predict(preprocess(data))) # выдает предсказание цены
...
class PriceConverter():
    ...
    def __init__(self):
        ...
        self.exchanges = {"US": us_exchange, "UK": uk_exchange, "JAP": jap_exchange, "JPY": jap_exchange, "EURO": euro_exchange, "ZAR": zar_exchange, "HK": hk_exchange,
                          "CHF": chf_exchange, "AU": au_exchange, "NOK": nok_exchange, "NT": nt_exchange, "MX": mx_exchange, "INR": inr_exchange, "ARS": ars_exchange,
                          "SEK": sek_exchange, "PHP": php_exchange, "CZK": czk_exchange, "KRW": krw_exchange, "NZ": nz_exchange, "S": s_exchange, "C": c_exchange,
                          "BRL": brl_exchange, "AED": aed_exchange, "DKK": dkk_exchange, "HUF": huf_exchange, "IDR": idr_exchange, "MAD": mad_exchange, "NGN": ngn_exchange,
                          "PLN": pln_exchange, "RM": rm_exchange, "CN¥": cn_exchange, "VEF": vef_exchange, "SKK": skk_exchange, "nan": 0}
    ...
    def return_numbers(self, t):
        ...
        while i < length:
            s_int = ''
            while i < length and ('0' <= t[i] <= '9' or t[i] == "." or t[i] == ","):
                s_int += t[i]
                i += 1
            i += 1
            if s_int != '':
                try:
                    integers.append(int("".join("".join(s_int.split(",")).split("."))))
                except ValueError:
                    return [0, 0]
        return integers
...
# import re
def parse_size(t: str) -> tuple[float, float]:
    """
        Функция для преобразования поля size(размер). Учитывает различные форматы
    """
    if ("cm" in t.splitlines()[0] or "mm" in t.splitlines()[0]):
        t = t.splitlines()[0]
    elif len(t.splitlines()) > 1 and ("cm" in t.splitlines()[1] or "mm" in t.splitlines()[1]):
        t = t.splitlines()[1]

    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? x (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? cm", t) # AA.a x BB.b cm
    if len(x) == 1:
        return float(x[0].split("x")[0]), float(x[0].split("x")[1][:-3])
    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? x (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?cm", t) # AA.a x BB.bcm
    if len(x) == 1:
        return float(x[0].split("x")[0]), float(x[0].split("x")[1][:-2])
    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? × (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?cm", t) # AA.a × BB.bcm
    if len(x) == 1:
        return float(x[0].split("×")[0]), float(x[0].split("×")[1][:-2])
    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? × (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? cm", t) # AA.a × BB.b cm
    if len(x) == 1:
        return float(x[0].split("×")[0]), float(x[0].split("×")[1][:-3])
    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? by (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? cm", t) # AA.a by BB.b cm
    if len(x) == 1:
        return float(x[0].split("by")[0]), float(x[0].split("by")[1][:-3])
    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? x (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? mm", t) # AA.a x BB.b mm
    if len(x) == 1:
        return float(x[0].split("x")[0]) / 100, float(x[0].split("x")[1][:-3]) / 100
    x = re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)? x (?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?mm", t) # AA.a x BB.bmm
    if len(x) == 1:
        return float(x[0].split("x")[0]) / 100, float(x[0].split("x")[1][:-2]) / 100
    return np.NaN, np.NaN

# from string import punctuation
def parse_medium(t: str) -> tuple[str, str]:
    """
    ...
    t = t.lower()
    if "on" in t:
        return t.split("on")[0].strip(punctuation + " "), t.split("on")[1].strip(punctuation + " ")
    if "," in t:
        return t.split(",")[0].strip(punctuation + " "), t.split(",")[1].strip(punctuation + " ")
    if "with" in t:
        return t.split("with")[0].strip(punctuation + " "), t.split("with")[1].strip(punctuation + " ")
    if "in" in t:
        return t.split("in")[0].strip(punctuation + " "), t.split("in")[1].strip(punctuation + " ")
    if "and" in t:
        return t.split("and")[0].strip(punctuation + " "), t.split("and")[1].strip(punctuation + " ")
    if "lithograph" in t:
...
def birth_death(birth: str, death: str) -> tuple[int, int]:
    """
    ...
    if isinstance(birth, float) or isinstance(birth, int):
        ...
        birth = int(float(re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?", birth)[0]))
    ...
    if isinstance(death, float) or isinstance(death, int):
        ...
        death = int(float(re.findall(r"(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?", death)[0]))
